...
## Overview
The “Yes In My Backyard” (YIMBY) initiative supports zoning and development reforms that expand affordable housing and boost local economies. Cedar Rapids, IA, a YIMBY-aligned metro, shows how increased housing supply can lower rent burdens and raise wages. In contrast, Miami–Fort Lauderdale–West Palm Beach, FL, a more restrictive market, faces slower growth. Federal incentives tied to housing production would help communities like these unlock grants, support wage gains, and improve affordability.

## Proposed Congressional Sponsors (by hometown):
Sponsors from Cedar Rapids, IA (16300) & Miami–Fort Lauderdale–West Palm Beach, FL (33100)

## Key Findings
Housing and Income Snapshot
Cedar Rapids, IA (16300) has one of the lowest rent burdens at 70.8 and steady housing development, while Miami–Fort Lauderdale–West Palm Beach, FL (33100) faces a much higher burden of 144. Average wages in Cedar Rapids have grown consistently, rising 11% between 2022 and 2023, compared with 4% yearly in Miami. Combined with lower housing costs and steady development, Cedar Rapids offers more positive living conditions and stronger economic opportunities for residents than the Miami metro area.

## Potential Support from Key Occupations
Service-industry workers—including retail, hospitality, healthcare support, and food services—represent a substantial share of employment in both Cedar Rapids, IA (16300) and Miami–Fort Lauderdale–West Palm Beach, FL (33100). Policies that improve housing affordability and support wage growth can directly benefit these workers by reducing rent burdens and increasing disposable income, strengthening local labor markets and overall economic stability in both metro areas.

## Suggested Metrics for Federal Program Design
To ensure measurable outcomes and facilitate legal translation, metrics following are recommened:
- Housing Supply Growth: Number of housing units permitted per year. 
- Affordability Ratio: Share of households spending less than 30% of income on rent.
- Income & Economic Growth: Year-over-year wage growth relative to the national average.
- Population Growth: Percent change in total population to capture demand and community vitality.

## Conclusion
The experiences of Cedar Rapids, IA and Miami–Fort Lauderdale–West Palm Beach, FL demonstrate that proactive, YIMBY-aligned housing policies can lower rent burdens, support wage growth, and strengthen local economies. By sponsoring initiatives that encourage municipalities to adopt such reforms, policymakers can help communities unlock federal incentives, expand housing supply, and improve living conditions for residents. Supporting YIMBY-aligned policies offers a clear path to more affordable, equitable, and economically vibrant neighborhoods.


## Data Acquisition - Task 1: Data Import
```{r}
#| code-fold: true
#get US census data
...
RENT_BURDEN |>
  group_by(GEOID) |>
  summarise(avg_rent_burden = mean(rent_to_national_income_ratio, na.rm = TRUE)) |>
  slice_max(avg_rent_burden, n = 10)

HIGH_RENT_BURDEN |>
  left_join(GEOID_NAME, join_by(GEOID)) |>
  select (GEOID, NAME, avg_rent_burden) |>
  datatable(options=list(searching=FALSE, info=FALSE)) |>
    formatRound(c('avg_rent_burden'))
...
RENT_BURDEN |>
  group_by(GEOID) |>
  summarise(avg_rent_burden = mean(rent_to_national_income_ratio, na.rm = TRUE)) |>
  slice_min(avg_rent_burden, n = 10)
...
HIGH_RENT_BURDEN |>
  left_join(PERMITS, join_by(GEOID == CBSA)) |>
  left_join(GEOID_NAME, join_by(GEOID == GEOID)) |>
  datatable(options=list(searching=FALSE, info=FALSE)) 

# 33100 Miami-Fort Lauderdale-West Palm Beach, FL Metro Area -- avg rent burden is 144 means 44% more >> nimby
...
LOW_RENT_BURDEN |>
  ...
  left_join(GEOID_NAME, join_by(GEOID == GEOID)) |>
  datatable(options=list(searching=FALSE, info=FALSE)) 
# 16300 Cedar Rapids, IA Metro Area -- avg rent burden is 70.8 (among the lowest across the states) and steady housing development yearly >> yimby
```
...
WAGES_GEOID_NAME|>
  filter (GEOID == 16300) |>
  group_by(YEAR) |>
  mutate(tot_yr_employment = sum(EMPLOYMENT, na.rm = TRUE)) |>
  ungroup() |>
  mutate(ind_percentage = (EMPLOYMENT / tot_yr_employment) * 100) |>
  slice_max(ind_percentage, n=10) 
...
```{r}
WAGES_GEOID_NAME
yearly_wages_yimby <- WAGES_GEOID_NAME |>
  filter(GEOID == 16300) |>
  group_by(YEAR) |>
  summarise(yearly_tot_wage = sum(AVG_WAGE, na.rm = TRUE), .groups = "drop") |>
  arrange(YEAR) |>  # make sure years are in order
  mutate(
    pct_change = (yearly_tot_wage / lag(yearly_tot_wage) - 1) * 100, 
    avg_pct_change = mean(pct_change, na.rm = TRUE)
  ) 

yearly_wages_yimby

yearly_wages_nimby <- WAGES_GEOID_NAME |>
  filter(GEOID == 33100) |>
  group_by(YEAR) |>
  summarise(yearly_tot_wage = sum(AVG_WAGE, na.rm = TRUE), .groups = "drop") |>
  arrange(YEAR) |>  # make sure years are in order
...
![data relationship diagram](images/mp02_data_relationship_diagram){fig-alt="data relationship diagram" width=100% }
...
#prepare NYC & Los Angeles to highlight
# GEOID = 35620 NYC

HOUSEHOLDS_NYC_LA <-
HOUSEHOLDS |> 
  inner_join(GEOID_NAME, join_by(GEOID == GEOID))  |>
  mutate(isNYC_LA = case_when( state=='LA' ~ 1,  GEOID == 35620 ~ 1, TRUE ~ 0))

ggplot(
  HOUSEHOLDS_NYC_LA, 
  aes(x=year, y=households, color=NAME.x)) + 
  geom_point() +
  geom_line() +
  gghighlight(isNYC_LA > 0) +
  scale_y_log10(
    labels = label_number(big.mark = ","),
    breaks = scales::breaks_log(n = 5)
  )+
